...
| `Qwen CLI` | 已支持 | 识别 `~/.qwen/projects/*/chats/*.jsonl`，可展示 transcript、usage 和主题摘要 |
| `Roo Code` | 已支持 | 识别 `~/.config/Code/User/globalStorage/rooveterinaryinc.roo-cline/tasks/*/ui_messages.json`，并从相邻 history 补模型与项目元数据 |
...
| `Qwen CLI` | 已支持 | 识别用户级 `settings.json` 与同级 `.env`，推断 provider/base URL/model，并可按项目发现 `.qwen/settings.json` |
| `Roo Code` | 已支持 | 识别用户级 `roo-code-settings.json` 与同级 `mcp_settings.json/cline_mcp_settings.json`，并可按会话发现项目级 `.roo/mcp.json` |
...
- 会话支持面从 3 个扩到 9 个，不再只停在 `Codex / Claude Code / OpenCode`
...
- 配置审计已经扩到 9 个助手，`Qwen CLI / Roo Code` 现在都能显示真实 `provider`、`model`、endpoint、MCP、风险标记和脱敏凭据
- `GitHub Copilot CLI / Factory Droid / Qwen CLI / Roo Code` 现在会按会话 `projectPath` 派生项目级配置，项目覆盖层也能直接出现在配置面板里
- 首页和会话详情现在会展示 `Codex / Claude Code / OpenCode / Gemini CLI / OpenClaw / Qwen CLI / Roo Code` 的 usage / cost 汇总
...
- 直接在界面里修改 `GitHub Copilot CLI / Factory Droid / Gemini CLI / OpenClaw / Qwen CLI / Roo Code` 配置，并保留备份、回滚和审计痕迹
...
- 让桌面端和 CLI 共用同一套会话查询层，Tauri 查询 API 现在也支持 `assistant` 过滤、`limit/offset` 分页和 `sortBy/descending` 排序
- 用 `cargo run -- serve` 在本地拉起只读 REST API，暴露 `health/list/search/get/view/expand`，并可选接入 Bearer token
- 用 `/openapi.json` 直接查看本地 REST API 的 OpenAPI 3.1 契约，方便后续接脚本、Web 壳层和自动化校验
- 用 CLI 的 `--json` 显式切到紧凑机器模式，给脚本和自动化链路稳定消费
- 用 `cargo run -- mcp` 通过 `stdio` 暴露 `list_sessions/search_sessions/get_session` 三个 MCP tools
- 看最近谁做过导出、隔离、恢复，动作有没有留痕
- 在桌面窗口或浏览器调试环境里统一查看这些信息

## 当前还没完成的部分

- 语义搜索、hybrid ranking 和更大样本下的性能压测
- 新会话启动、更多助手控制编排，以及更细粒度的运行态诊断
- 建立在 worktree CLI 之上的多项目调度、并行 agent 编排、容器隔离执行
- provider 健康探测、自动切换、更多助手配置治理和云同步
- contribution graph、leaderboard 一类更深的 analytics 视图
- skills / prompts 管理、远程壳层和附件预览
...
- `junhoyeo/tokscale`
  已吸收：本地 usage / token / cost 字段模型和多助手聚合面板思路，已重写为 OSM 自己的实现，并补进 `Qwen CLI / Roo Code` 的 clean-room 会话解析
...
```bash
cargo run --manifest-path src-tauri/Cargo.toml -- serve --host 127.0.0.1 --port 43210
```
...
```powershell
$env:OPEN_SESSION_MANAGER_API_TOKEN = "osm-local-token"
cargo run --manifest-path src-tauri/Cargo.toml -- serve --host 127.0.0.1 --port 43210
```
...
```bash
cargo run --manifest-path src-tauri/Cargo.toml -- mcp --fixtures tests/fixtures
```
...
cargo run --manifest-path src-tauri/Cargo.toml -- list --fixtures tests/fixtures --json
```
