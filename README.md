...
- 配置审计已经扩到 8 个助手，`Qwen CLI` 现在也能显示真实 `provider`、`model`、endpoint、MCP、风险标记和脱敏凭据
- `GitHub Copilot CLI / Factory Droid / Qwen CLI` 现在会按会话 `projectPath` 派生项目级配置，项目覆盖层也能直接出现在配置面板里
...
- 直接在界面里修改 `GitHub Copilot CLI / Factory Droid / Gemini CLI / OpenClaw / Qwen CLI` 配置，并保留备份、回滚和审计痕迹
