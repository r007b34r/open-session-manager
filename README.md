...
| `GitHub Copilot CLI` | 已支持 | 识别 `~/.copilot/config.json`，联动同目录 `mcp-config.json`，展示 enterprise base URL、tool policy、MCP 与 masked token 风险 |
| `Factory Droid` | 已支持 | 识别 `settings.json` 并合并 `settings.local.json`，展示 provider/base URL、command allowlist、MCP 与 masked key 风险 |
