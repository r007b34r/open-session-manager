...
| `Qwen CLI` 会话解析 | 已实现 | 支持 `~/.qwen/projects/*/chats/*.jsonl`，会展示 transcript、usage 和 session topic |
| `Roo Code` 会话解析 | 已实现 | 支持 `~/.config/Code/User/globalStorage/rooveterinaryinc.roo-cline/tasks/*/ui_messages.json`，并可从同目录 `api_conversation_history.json` 补模型和工作区元数据 |
...
| `Qwen CLI` 配置审计 | 已实现 | 支持用户级 `settings.json` + 同级 `.env`，可从 `modelProviders` 推断 provider/base URL/env key，并能按会话派生项目级 `.qwen/settings.json` |
| `Roo Code` 配置审计 | 已实现 | 支持用户级 `roo-code-settings.json` + 同级 `mcp_settings.json/cline_mcp_settings.json`，可解析当前 profile 的 provider/base URL/model/key，并能按会话派生项目级 `.roo/mcp.json` |
...
| `GitHub Copilot CLI / Qwen CLI / Roo Code / Factory Droid / Gemini CLI / OpenClaw` 安全写回 | 已实现 | 支持可视化编辑、输入校验、备份 manifest、回滚测试、审计事件，并可在 Audit 页查看备份 manifest 路径；`Roo Code` 当前直接编辑 active profile，不做 provider preset/rename |
...
| Browser preview | 已实现 | `npm --prefix web run browser` 会先 build 再以 `vite preview` 固定在 `127.0.0.1:4173` 启动，Playwright E2E 复用同一条链路 |
...
| Usage / cost analytics | 已实现 | `Codex / Claude Code / OpenCode / Gemini CLI / OpenClaw / Qwen CLI / Roo Code` 已展示会话级和总览级 token/cost 汇总、本地价格目录估算、`reported / estimated / unknown` 成本来源和日级 usage timeline |
...
| `junhoyeo/tokscale` | 已吸收 | 本地 usage / token / cost 聚合面板、成本来源标注、本地价格目录估算与 usage timeline 已落进 OSM clean-room 实现，且已补上 `Qwen CLI / Roo Code` 会话日志结构的 clean-room 解析 |
