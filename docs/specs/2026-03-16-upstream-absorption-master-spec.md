...
- 已吸收：统一 provider/config 治理方向，以及 `Gemini CLI / OpenClaw` 配置路径、provider、base URL、auth mode、风险审计
- 已落地：`Gemini CLI / GitHub Copilot CLI / Factory Droid / OpenClaw` 配置治理读取、项目级配置发现和风险展示
...
- 配置审计：`Codex / Claude Code / OpenCode / Gemini CLI / GitHub Copilot CLI / Factory Droid / OpenClaw`
...
- 已有 7 个助手的配置审计读取，且 `GitHub Copilot CLI / Factory Droid / Gemini CLI / OpenClaw` 已具备安全写回链路，但仍缺 provider presets 和健康治理
...
- 基于配置治理竞品与官方资料，补齐 `GitHub Copilot CLI / Factory Droid` 用户级配置审计
- 在 Rust snapshot、CLI snapshot、Web UI 中都能看到新增 usage/config 数据
...
- token 定价在线拉取
- 多账号 usage 聚合
- provider presets / shared snippets / health monitor
- HTTP / MCP 平台化
- 会话恢复控制
...
1. Rust 单测先失败再转绿，覆盖 usage 解析和 Copilot/Factory 配置审计
2. `cargo test --test cli_snapshot` 能看到 usage 与新增 config 字段
3. Web 单测能看到 usage/cost 面板以及 Copilot/Factory 配置卡片
