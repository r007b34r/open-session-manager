...
- `Codex / Claude Code / GitHub Copilot CLI / OpenCode` 的真实 resume / continue 控制链路，以及 Web 详情页的一键恢复入口
- 基础 usage / cost 聚合
...
| `SES-02` | P0 | partial | 恢复既有会话 | 当前已支持 `Codex / Claude Code / GitHub Copilot CLI / OpenCode` 真实 resume，snapshot、HTTP API 和 UI 可观测最近结果；其余助手待补控制适配器 |
| `SES-03` | P0 | partial | 继续运行中的会话 | 当前已支持 `Codex / Claude Code / GitHub Copilot CLI / OpenCode` continue prompt 并落审计，附着与节流保护已完成；其余助手待补控制适配器 |
...
| `SES-07` | P0 | partial | one-click resume | Web 详情页已接恢复按钮、继续提示和最近控制结果；真实执行目前覆盖 `Codex / Claude Code / GitHub Copilot CLI / OpenCode` |
